...
// --- DEEL 1      van 3 dit code blok    --- (<<< REVISE: Added firstEnemyLanded flag declaration >>>)
...
const
    ...
    // CS_ENTRANCE_PATH_SPEED - Defined in part 2 now to avoid redeclaration error trace
    BASE_CS_SPEED_MULTIPLIER = 4.2, // Verhoogde waarde uit vorige revisie
    MAX_CS_SPEED_MULTIPLIER = 5.0,  // Verhoogde waarde uit vorige revisie
    ...
    // <<< GEWIJZIGD: Grid Firing & Return Speed Scaling (Values from previous step) >>>
    LEVEL_CAP_FOR_SCALING = 50, // Max level for scaling (remains)
    // Grid Firing Scaling
    BASE_GRID_FIRE_INTERVAL = 2000, // Was 3500, Nu sneller checken
    MIN_GRID_FIRE_INTERVAL = 800,   // Was 800, Iets sneller op max
    BASE_GRID_FIRE_PROBABILITY = 0.05, // Was 0.02, Verdubbelde startkans
    MAX_GRID_FIRE_PROBABILITY = 0.15,  // Was 0.15, Iets hogere max kans
    BASE_GRID_MAX_FIRING_ENEMIES = 5,  // Was 5, Meer vijanden tegelijk in begin
    MAX_GRID_MAX_FIRING_ENEMIES = 15,   // Was 14, Iets meer op max
    // Return Speed Scaling
    BASE_RETURN_SPEED_FACTOR = 1.5, // Multiplier for BASE_RETURN_SPEED at level 1 (relative to ENTRANCE_SPEED)
    MAX_RETURN_SPEED_FACTOR = 2.5   // Max multiplier at level 50+
;
// <<< EINDE WIJZIGING GRID FIRING >>>


// --- Globale State Variabelen ---
// ... (rest van variabelen ongewijzigd) ...
...
let lastEnemyDetachTime = 0; let gridMoveDirection = 1; let gridMoveSpeed = 0.3; // gridMoveSpeed wordt nu overschreven door scaling
let lastGridFireCheckTime = 0;
// <<< NIEUW: Vlag om eerste landing te detecteren voor timer reset >>>
...
let isShowingResultsScreen = false; // <<< NU alleen gebruikt voor interne logica/render state >>>
...
let tripleAttackSound; // Wordt geïnitialiseerd in initializeDOMElements
...
// <<< GEWIJZIGD: GRID_HORIZONTAL_MARGIN_PERCENT verkleind. >>>
...
const
    ...
    BASE_ENEMY_BULLET_SPEED = 9,     // Start kogelsnelheid
    MAX_ENEMY_BULLET_SPEED = 14,    // Maximale kogelsnelheid op/na level 50
    BASE_ENEMY_ATTACK_SPEED = 5.5,   // Start duiksnelheid basis
    MAX_ENEMY_ATTACK_SPEED = 8.0,    // Maximale duiksnelheid basis op/na level 50
    BASE_MAX_ATTACKING_ENEMIES = 4,  // Start max aantal aanvallers
    MAX_MAX_ATTACKING_ENEMIES = 10,   // Max aantal aanvallers op/na level 50
    BASE_GRID_MOVE_SPEED = 0.3,    // Start *enemy* grid horizontale snelheid
    MAX_GRID_MOVE_SPEED = 0.7,     // Max *enemy* grid horizontale snelheid op/na level 50
    BASE_GRID_BREATH_CYCLE_MS = 2000, // Start grid adem cyclus (langer = langzamer)
    MIN_GRID_BREATH_CYCLE_MS = 1000, // Minimale grid adem cyclus (korter = sneller) op/na level 50
    BASE_ENEMY_BULLET_BURST_COUNT = 1, // Start aantal kogels per burst
    MAX_ENEMY_BULLET_BURST_COUNT = 5,  // Max aantal kogels per burst op/na level 50
    BASE_ENEMY_AIM_FACTOR = 0.95, // Start mik-factor (0 = recht omlaag, 1 = perfect op speler)
    MAX_ENEMY_AIM_FACTOR = 0.100,  // Maximale mik-factor op/na level 50
    ...
    ENTRANCE_SPEED = 6, // Basis snelheid voor entrance & *ook gebruikt als basis voor return*
    BASE_RETURN_SPEED = ENTRANCE_SPEED, // Base speed for returning enemies (same as entrance for now)
    ...
    // BASE_CS_SPEED_MULTIPLIER etc. moved to beginning of constants
    // CS Berichten etc...
    ...
    // AI Constanten... (Geen wijzigingen in DIT deel)
    AI_SHOOT_COOLDOWN = 140, AI_STABILIZATION_DURATION = 500, AI_POSITION_MOVE_SPEED_FACTOR = 1.2,
    AI_COLLISION_LOOKAHEAD = SHIP_HEIGHT * 3.5, AI_COLLISION_BUFFER = SHIP_WIDTH * 0.6, // Base buffer
    FINAL_DODGE_LOOKAHEAD = AI_COLLISION_LOOKAHEAD * 4.5, // Was 3.5
    FINAL_DODGE_BUFFER_BASE = AI_COLLISION_BUFFER * 3.5, // Was 2.8
    ENTRANCE_BULLET_DODGE_LOOKAHEAD = FINAL_DODGE_LOOKAHEAD * 1.8, // Was 1.5 (relative to NEW final)
    ENTRANCE_BULLET_DODGE_BUFFER = FINAL_DODGE_BUFFER_BASE * 2.2, // Was 1.8 (relative to NEW final)
    FINAL_AI_DODGE_MOVE_SPEED_FACTOR = 3.8, // Speed factor during dodge (niet direct gebruikt in X target calc)
...
function initializeDOMElements() {
    ...
    try {
        ...
        // <<< GEWIJZIGD: Volumes onder elkaar gezet >>>
        if(playerShootSound)     playerShootSound.volume = 0.4;
        if(explosionSound)       explosionSound.volume = 0.4; // Kill.mp3
        ...
        if(dualShipSound)        dualShipSound.volume = 0.4; // Was coin.mp3
...
// --- START OF FILE setup_utils.js ---
// --- DEEL 3      van 3 dit code blok    --- (<<< REVISE: Key bindings for Backspace/Enter >>>)
// <<< GEWIJZIGD: Logs uit triggerFinalGameOverSequence en gamepad handlers verwijderd. >>>
// <<< GEWIJZIGD: Log uit saveHighScore verwijderd. >>>
// <<< GEWIJZIGD: getCurrentGridSlotPosition gebruikt nu currentVerticalSpacing voor Y calc. >>>
// <<< GEWIJZIGD: Logs uit togglePause() verwijderd. >>>
// <<< GEWIJZIGD: Escape/Enter -> Escape/Backspace for menu return. Enter added for firing. >>>
...
*/
function getCurrentGridSlotPosition(gridRow, gridCol, enemyWidth) { /* ... ongewijzigd ... */ if (!gameCanvas || gameCanvas.width === 0 || gridRow < 0 || gridCol < 0) { return { x: gameCanvas?.width / 2 || 200, y: ENEMY_TOP_MARGIN || 100 }; } const baseEnemyWidthForCalc = ENEMY_WIDTH; let currentHorizontalSpacing = ENEMY_H_SPACING_FIXED; let currentVerticalSpacing = ENEMY_V_SPACING; if (GRID_BREATH_ENABLED && isGridBreathingActive) { const extraHSpacing = ENEMY_H_SPACING_FIXED * GRID_BREATH_MAX_EXTRA_H_SPACING_FACTOR * currentGridBreathFactor; currentHorizontalSpacing = ENEMY_H_SPACING_FIXED + extraHSpacing; const extraVSpacing = ENEMY_V_SPACING * GRID_BREATH_MAX_EXTRA_V_SPACING_FACTOR * currentGridBreathFactor; currentVerticalSpacing = ENEMY_V_SPACING + extraVSpacing; } const actualGridWidth = GRID_COLS * baseEnemyWidthForCalc + (GRID_COLS - 1) * currentHorizontalSpacing; const initialGridStartX = Math.round((gameCanvas.width - actualGridWidth) / 2); const currentStartX = initialGridStartX + currentGridOffsetX; const colStartX = currentStartX + gridCol * (baseEnemyWidthForCalc + currentHorizontalSpacing); const centeringOffset = (baseEnemyWidthForCalc - enemyWidth) / 2; const targetX = Math.round(colStartX + centeringOffset); const targetY = Math.round(ENEMY_TOP_MARGIN + gridRow * (ENEMY_HEIGHT + currentVerticalSpacing)); return { x: targetX, y: targetY }; }
...
/**
 * Handles keydown events for game control and menu navigation.
 * <<< GEWIJZIGD: Menu return is nu Escape/Backspace. Enter is toegevoegd voor vuren. >>>
 */
function handleKeyDown(e) {
    try {
        // <<< GEWIJZIGD: Backspace toegevoegd aan relevante toetsen >>>
        const relevantKeys = [" ", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Enter", "Escape", "Backspace", "w", "a", "s", "d", "p", "P", "j", "J", "l", "L", "i", "I", "Numpad4", "Numpad6", "Numpad0"];
        ...
        if (isInGameState) { // We zijn in het spel
            if ((e.key === 'p' || e.key === 'P') && playerLives > 0 && gameOverSequenceStartTime === 0 && !isShowingPlayerGameOverMessage) {
            ...
            if (!isPaused) {
                if (playerLives > 0) {
                    if (!isManualControl) { // AI Demo
                        // <<< GEWIJZIGD: Exit met Escape of Backspace >>>
                        if (e.key === "Escape" || e.key === "Backspace") {
                            if(typeof stopGameAndShowMenu === 'function') stopGameAndShowMenu();
                        } else if (!e.altKey && !e.ctrlKey && !e.metaKey && !e.shiftKey) { // Any other key stops demo
                            if(typeof showMenuState === 'function') showMenuState();
                        }
                    } else { // Manual Control (Player(s))
                        switch (e.code) {
                            // P1 Movement
                            case "ArrowLeft": case "KeyA": keyboardP1LeftDown = true; break;
                            case "ArrowRight": case "KeyD": keyboardP1RightDown = true; break;
                            // P1 Fire (Original Keys)
                            case "Space": case "ArrowUp": case "KeyW": keyboardP1ShootDown = true; break;
                            // P2 Movement
                            case "KeyJ": if(isTwoPlayerMode) keyboardP2LeftDown = true; break;
                            case "KeyL": if(isTwoPlayerMode) keyboardP2RightDown = true; break;
                             case "Numpad4": if(isTwoPlayerMode) keyboardP2LeftDown = true; break;
                             case "Numpad6": if(isTwoPlayerMode) keyboardP2RightDown = true; break;
                            // P2 Fire (Original Keys)
                            case "KeyI": if(isTwoPlayerMode) keyboardP2ShootDown = true; break;
                            case "Numpad0": if(isTwoPlayerMode) keyboardP2ShootDown = true; break;

                            // <<< NIEUW: Enter voor vuren (huidige speler) >>>
                            case "Enter":
                                if (isTwoPlayerMode) {
                            ...
                            // <<< GEWIJZIGD: Terug naar menu met Escape of Backspace >>>
                            case "Escape":
                            case "Backspace":
                                if(typeof stopGameAndShowMenu === 'function') stopGameAndShowMenu();
                                break;
                        ...
                        // Fallback for e.key (if e.code fails for some reason) - P2 only
                        if (!keyboardP2LeftDown && isTwoPlayerMode && e.key === "j") keyboardP2LeftDown = true;
                        if (!keyboardP2RightDown && isTwoPlayerMode && e.key === "l") keyboardP2RightDown = true;
                        if (!keyboardP2ShootDown && isTwoPlayerMode && e.key === "i") keyboardP2ShootDown = true;
                    }
                }
            }
        } else { // We zijn in het Menu of Score Screen
            if (isShowingScoreScreen) {
                if (!e.altKey && !e.ctrlKey && !e.metaKey && !e.shiftKey && e.key !== 'p' && e.key !== 'P') {
                    if(typeof showMenuState === 'function') showMenuState(); // Any key exits score screen
                    return;
                }
            } else { // Menu navigatie
                stopAutoDemoTimer();
                switch (e.key) {
                    ...
                    case "Enter": case " ": // Selecteren met Enter of Spatie
                        if (isFiringModeSelectMode) { if (selectedButtonIndex === 0) { selectedFiringMode = 'rapid'; } else { selectedFiringMode = 'single'; } baseStartGame(true); }
                        else if (isPlayerSelectMode) { if (selectedButtonIndex === 0) { startGame1P(); } else { startGame2P(); } }
                        else { if (selectedButtonIndex === 0) { isPlayerSelectMode = true; isFiringModeSelectMode = false; selectedButtonIndex = 0; startAutoDemoTimer(); } else if (selectedButtonIndex === 1) { exitGame(); } }
                        break;
                    case "Escape": // Terug/Annuleren met Escape
                        if (isFiringModeSelectMode) { isFiringModeSelectMode = false; isPlayerSelectMode = true; selectedButtonIndex = 0; startAutoDemoTimer(); }
                        else if (isPlayerSelectMode) { isPlayerSelectMode = false; isFiringModeSelectMode = false; selectedButtonIndex = 0; startAutoDemoTimer(); }
                        else { exitGame(); } // In hoofdmenu -> Exit
                        break;
                     case "Backspace": // <<< GEWIJZIGD: Backspace doet hetzelfde als Escape in het menu >>>
                         if (isFiringModeSelectMode) { isFiringModeSelectMode = false; isPlayerSelectMode = true; selectedButtonIndex = 0; startAutoDemoTimer(); }
                         else if (isPlayerSelectMode) { isPlayerSelectMode = false; isFiringModeSelectMode = false; selectedButtonIndex = 0; startAutoDemoTimer(); }
                         else { exitGame(); } // In hoofdmenu -> Exit
                         break;
                     // <<< EINDE GEWIJZIGD >>>
                    default: startAutoDemoTimer(); break; // Andere toetsen starten alleen timer opnieuw
                }
            }
        }
    } catch(err) {
...
function handleKeyUp(e) { /* ... ongewijzigd ... */ try { switch (e.code) { case "ArrowLeft": case "KeyA": keyboardP1LeftDown = false; break; case "ArrowRight": case "KeyD": keyboardP1RightDown = false; break; case "Space": case "ArrowUp": case "KeyW": keyboardP1ShootDown = false; break; case "KeyJ": keyboardP2LeftDown = false; break; case "KeyL": keyboardP2RightDown = false; break; case "KeyI": keyboardP2ShootDown = false; break; case "Numpad4": keyboardP2LeftDown = false; break; case "Numpad6": keyboardP2RightDown = false; break; case "Numpad0": keyboardP2ShootDown = false; break; case "Enter": /* <<< NIEUW: Reset beide shoot flags bij loslaten Enter >>> */ keyboardP1ShootDown = false; keyboardP2ShootDown = false; break; } if (keyboardP2LeftDown && e.key === "j") keyboardP2LeftDown = false; if (keyboardP2RightDown && e.key === "l") keyboardP2RightDown = false; if (keyboardP2ShootDown && e.key === "i") keyboardP2ShootDown = false; } catch(err) { console.error("Error in handleKeyUp:", err); keyboardP1LeftDown = false; keyboardP1RightDown = false; keyboardP1ShootDown = false; keyboardP2LeftDown = false; keyboardP2RightDown = false; keyboardP2ShootDown = false; } }

// --- Gamepad Event Handlers ---
/**
 * <<< GEWIJZIGD: Logs over gamepad connect/disconnect verwijderd. >>>
 */
function handleGamepadConnected(event) { /* ... ongewijzigd in logic ... */ try { if (connectedGamepadIndex === null) { connectedGamepadIndex = event.gamepad.index; const numButtons = event.gamepad.buttons.length; previousButtonStates = new Array(numButtons).fill(false); previousDemoButtonStates = new Array(numButtons).fill(false); previousGameButtonStates = new Array(numButtons).fill(false); if (!isInGameState) { stopAutoDemoTimer(); selectedButtonIndex = 0; } } else if (connectedGamepadIndexP2 === null) { connectedGamepadIndexP2 = event.gamepad.index; const numButtons = event.gamepad.buttons.length; previousGameButtonStatesP2 = new Array(numButtons).fill(false); } else { } } catch(e) { console.error("Error in handleGamepadConnected:", e); } }
function handleGamepadDisconnected(event) { /* ... ongewijzigd in logic ... */ try { if (connectedGamepadIndex === event.gamepad.index) { connectedGamepadIndex = null; previousButtonStates = []; previousDemoButtonStates = []; previousGameButtonStates = []; if (!isInGameState) { selectedButtonIndex = -1; joystickMovedVerticallyLastFrame = false; startAutoDemoTimer(); } p1FireInputWasDown = false; } else if (connectedGamepadIndexP2 === event.gamepad.index) { connectedGamepadIndexP2 = null; previousGameButtonStatesP2 = []; p2FireInputWasDown = false; } } catch(e) { console.error("Error in handleGamepadDisconnected:", e); p1JustFiredSingle = false; p2JustFiredSingle = false; p1FireInputWasDown = false; p2FireInputWasDown = false; } }

// --- High Score ---
/**
 * <<< GEWIJZIGD: Log over new high score verwijderd. >>>
 */
function saveHighScore() { /* ... ongewijzigd in logic ... */ try { let potentialNewHighScore = 0; if (isTwoPlayerMode) { potentialNewHighScore = Math.max(player1Score, player2Score); } else { potentialNewHighScore = score; } if (isManualControl && potentialNewHighScore > highScore) { highScore = potentialNewHighScore; } } catch (e) { console.error("Error in saveHighScore:", e); } }
 ...
 * <<< GEWIJZIGD: Logs over pauze/resume verwijderd. >>>
*/
function togglePause() { /* ... ongewijzigd in logic ... */ if (!isInGameState || playerLives <= 0 || gameOverSequenceStartTime > 0 || isShowingPlayerGameOverMessage ) return; isPaused = !isPaused; if (isPaused) { pauseAllSounds(); clearTimeout(mouseIdleTimerId); mouseIdleTimerId = null; } else { resumeAllSounds(); clearTimeout(mouseIdleTimerId); mouseIdleTimerId = setTimeout(hideCursor, 2000); } }
 ...
 * <<< GEWIJZIGD: Start de timer anders voor 2P om "GAME OVER" tekst over te slaan >>>
 * <<< GEWIJZIGD: Logs voor 1P/2P start verwijderd. >>>
 */
function triggerFinalGameOverSequence() { /* ... ongewijzigd in logic ... */ if (isInGameState && gameOverSequenceStartTime === 0) { isPaused = false; isShowingDemoText = false; isShowingIntro = false; isWaveTransitioning = false; showCsHitsMessage = false; showExtraLifeMessage = false; showPerfectMessage = false; showCSClearMessage = false; showCsHitsForClearMessage = false; showCsScoreForClearMessage = false; showReadyMessage = false; showCsBonusScoreMessage = false; isShowingPlayerGameOverMessage = false; isEntrancePhaseActive = false; isCsCompletionDelayActive = false; csCompletionDelayStartTime = 0; csCompletionResultIsPerfect = false; csIntroSoundPlayed = false; if (isManualControl) { saveHighScore(); } const soundsToStop = [ gridBackgroundSound, entranceSound, butterflyDiveSound, startSound, coinSound, bossGalagaDiveSound, enemyShootSound, levelUpSound, playerShootSound, extraLifeSound, csPerfectSound, csClearSound, explosionSound, waveUpSound, menuMusicSound, readySound, tripleAttackSound, captureSound, shipCapturedSound, dualShipSound, hiScoreSound ]; soundsToStop.forEach(sound => stopSound(sound)); isGridSoundPlaying = false; if (typeof enemies !== 'undefined' && Array.isArray(enemies)) { enemies.forEach(e => { if (e?.state === 'attacking') { if(e.type === ENEMY3_TYPE) stopSound(bossGalagaDiveSound); else stopSound(butterflyDiveSound); } }); } const now = Date.now(); if (!isTwoPlayerMode) { playSound(gameOverSound); gameOverSequenceStartTime = now; } else { gameOverSequenceStartTime = now - GAME_OVER_DURATION; } bullets = []; enemyBullets = []; explosions = []; fallingShips = []; isDualShipActive = false; isShowingResultsScreen = false; previousButtonStates = []; previousGameButtonStates = []; previousGameButtonStatesP2 = []; } }
